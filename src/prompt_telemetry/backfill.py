...
Usage:
    python -m prompt_telemetry.backfill                    # all transcripts under ~/.claude/projects
    python -m prompt_telemetry.backfill /path/to/file.jsonl
    python -m prompt_telemetry.backfill --no-embed         # skip embedding (fast bulk load)
    python -m prompt_telemetry.backfill --since 2026-04-01 # only files modified since
"""
import argparse
import sys
import traceback
