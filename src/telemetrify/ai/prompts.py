...
CLASSIFY_PORTS = PromptTemplate(
    version="classify-ports-v2",
    model=MODEL_HAIKU,
    system=(
        "You identify network services from minimal evidence on a "
        "multi-user GPU compute server (Linux, university lab box, "
        "running PyTorch / vLLM / Jupyter / training jobs).\n"
        "\n"
        "Default port-knowledge table — apply liberally when the port matches:\n"
        "  5555  ZeroMQ default (binary; non-HTTP)\n"
        "  6379  Redis\n"
        "  5432  Postgres\n"
        "  3306  MySQL\n"
        "  6006  TensorBoard\n"
        "  8000-8099  vLLM or generic FastAPI/uvicorn\n"
        "  8765-8767  telemetrify FastAPI\n"
        "  8888-8890  Jupyter Lab/Notebook\n"
        "  11434  Ollama\n"
        "  9090  Prometheus\n"
        "  9100  node_exporter\n"
        "  4040  Ray dashboard\n"
        ...
        "  29500-29510  torch.distributed init / training-rpc\n"
        "  30000-49999  ephemeral high ports — overwhelmingly "
        "  Jupyter/IPython kernel ZMQ sockets, Ray workers, or "
        "  PyTorch dist RPC sub-channels on this kind of host\n"
        "\n"
        "Banner-fingerprint hints:\n"
        "  'non-http: RemoteDisconnected' / 'Empty reply' → binary "
        "  protocol (ZMQ / gRPC / Redis) — cross-reference port.\n"
        "  'HTTP/1.1 404' + small Content-Length + text/plain → "
        "  often a Go HTTP server (Prometheus client_golang, Ray "
        "  HTTP head).\n"
        "  'HTTP/1.1 403' + Content-Length ~46 → auth-walled "
        "  service (Jupyter, MLflow, token-protected Flask).\n"
        "  'application/json' Content-Type → JSON API.\n"
        "  'Server: vllm' → vllm. 'Server: nginx' → reverse proxy.\n"
        "\n"
        "BE GROUNDED. Mark confidence:high ONLY when port-number or "
        "banner gives a strong signal. For high ports (30000-49999) "
        "with no other signal, prefer kind=`ipython-kernel-zmq` "
        "confidence=`medium` over a lazy `unknown / low` — they're "
        "overwhelmingly notebook kernel sockets.\n"
        "\n"
        "Return ONLY a single JSON object — no markdown, no prose, "
        "no triple-backticks. The very first character MUST be `{`."
    ),
    user_template=(
        "Classify these listening ports on the lab GPU server:\n\n"
        "{ports_block}\n\n"
        ...
        "    {{\n"
        "      \"port\": <int>,\n"
        "      \"kind\": \"<short lowercase canonical label>\",\n"
        ...
        "}}\n\n"
        "Example inputs:\n"
        "  - port 5555  probe: 'non-http: RemoteDisconnected'\n"
        "  - port 11434 probe: 'HTTP/1.1 200\\nContent-Type: application/json'\n"
        "Example output (your response must look exactly like this):\n"
        "{{\"classifications\":["
        "{{\"port\":5555,\"kind\":\"zmq\",\"label\":\"ZeroMQ\","
        "\"confidence\":\"high\",\"reasoning\":\"ZMQ default port; "
        "non-HTTP binary banner\"}},"
        "{{\"port\":11434,\"kind\":\"ollama\",\"label\":\"Ollama\","
        "\"confidence\":\"high\",\"reasoning\":\"Ollama default port; "
        "JSON API\"}}"
        "]}}"
    ),
)
