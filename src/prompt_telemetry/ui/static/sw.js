/* prompt-telemetry — Web Push service worker.
 *
...
self.addEventListener("push", (event) => {
  ...
  if (event.data) {
    try {
      ...
      data = { title: "prompt-telemetry", body: event.data.text() };
    }
  }

  const title = data.title || "prompt-telemetry";
  const options = {
    body: data.body || "",
    icon: NOTIF_ICON,
    badge: NOTIF_ICON,
    tag: data.tag || "prompt-telemetry",
...
self.addEventListener("notificationclick", (event) => {
  ...
  event.waitUntil(
    (async () => {
      ...
      // Otherwise just focus any prompt-telemetry tab; failing that, open one.
      for (const client of allClients) {
        try {
