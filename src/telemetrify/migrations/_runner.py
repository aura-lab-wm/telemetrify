...
Migration files live in `telemetrify/migrations/NNN_name.{sql,py}` and are
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point: `python -m telemetrify.migrations apply` / `status`."""
    ...
    if cmd == "status":
        ...
        print(f"unknown command: {cmd}\nusage: python -m telemetrify.migrations [apply|status]")
        return 2
